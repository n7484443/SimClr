...
def train(base_optimizer, hyper_batch_size, hyper_batch_size_predictor, hyper_epoch, hyper_epoch_predictor, lr,
          lr_predictor, sam=False):
    ...
    projection_dim = 128
    class_size = 10
    simclr = SimCLR(base_encoder=torchvision.models.resnet50, projection_dim=projection_dim).to(device)
    simclr.train()
    ...
    if sam:
        optimizer = SAM(simclr.parameters(), lr=lr, base_optimizer=base_optimizer)
    else:
        optimizer = base_optimizer(simclr.parameters(), lr=lr)
    ...
    for epoch in range(1, hyper_epoch + 1):
        ...
        sleep(0.1)
        simclr_x.append((loss_sum_train, loss_sum_test))
        if epoch >= 10:
            scheduler.step()
    ...
    for epoch in range(1, hyper_epoch_predictor + 1):
        ...
        loss_sum_train /= len(trainLoader)
        loss_sum_test /= len(testLoader)
        eval_x.append((loss_sum_train, loss_sum_test, 100.0 * correct / total_size))
        tqdm.write('Avg Loss : %.4f Validation Loss : %.4f Learning Late: %.4f' % (
            loss_sum_train, loss_sum_test, scheduler.get_last_lr()[0]))
        sleep(0.1)
        if epoch >= 10:
    ...
    return simclr_x, eval_x, correct_1, correct_5
...
if __name__ == '__main__':
    ...
    hyper_epoch = 20
    hyper_epoch_predictor = 100
    lr = 0.075 * math.sqrt(hyper_batch_size)
    lr_predictor = 1e-3

    (sam_simclr, sam_eval, sam_correct_1, sam_correct_5) = train(base_optimizer=torch.optim.SGD,
                                                                 hyper_epoch=hyper_epoch,
                                                                 hyper_epoch_predictor=hyper_epoch_predictor,
                                                                 hyper_batch_size=hyper_batch_size,
                                                                 hyper_batch_size_predictor=hyper_batch_size_predictor,
                                                                 lr=lr, lr_predictor=lr_predictor,
                                                                 sam=True)
    (lars_simclr, lars_eval, lars_correct_1, lars_correct_5) = train(base_optimizer=optim.LARS, hyper_epoch=hyper_epoch,
                                                                     hyper_epoch_predictor=hyper_epoch_predictor,
                                                                     hyper_batch_size=hyper_batch_size,
                                                                     hyper_batch_size_predictor=hyper_batch_size_predictor,
                                                                     lr=lr, lr_predictor=lr_predictor)
    (sgd_simclr, sgd_eval, sgd_correct_1, sgd_correct_5) = train(base_optimizer=torch.optim.SGD,
                                                                 hyper_epoch=hyper_epoch,
                                                                 hyper_epoch_predictor=hyper_epoch_predictor,
                                                                 hyper_batch_size=hyper_batch_size,
                                                                 hyper_batch_size_predictor=hyper_batch_size_predictor,
    ...
    range_simclr_x = np.arange(1, hyper_epoch + 1, 1)
    range_eval_x = np.arange(1, hyper_epoch_predictor + 1, 1)
    fig, (ax1, ax2) = plt.subplots(2, 1)
    ax1.plot(range_simclr_x, [x[0] for x in sam_simclr], label='SAM Training loss', color='C0', linestyle='solid')
    ax1.plot(range_simclr_x, [x[1] for x in sam_simclr], label='SAM Validation loss', color='C0', linestyle='dashed')
    ax1.plot(range_simclr_x, [x[0] for x in lars_simclr], label='LARS Training loss', color='C1', linestyle='solid')
    ax1.plot(range_simclr_x, [x[1] for x in lars_simclr], label='LARS Validation loss', color='C1', linestyle='dashed')
    ax1.plot(range_simclr_x, [x[0] for x in sgd_simclr], label='SGD Training loss', color='C2', linestyle='solid')
    ax1.plot(range_simclr_x, [x[1] for x in sgd_simclr], label='SGD Validation loss', color='C2', linestyle='dashed')
    ax1.set_yscale('log')
    ax1.legend()
    ax1.set_title(f'lr : {lr} epoch : {hyper_epoch} batch_size : {hyper_batch_size}')

    ax2a = ax2.twinx()
    ax2.plot(range_eval_x, [x[0] for x in sam_eval], label='SAM Training loss', color='C0', linestyle='solid')
    ax2.plot(range_eval_x, [x[1] for x in sam_eval], label='SAM Validation loss', color='C0', linestyle='dashed')
    ax2a.plot(range_eval_x, [x[2] for x in sam_eval], label='SAM Accuracy', color='C0', linestyle='dotted')
    ax2.plot(range_eval_x, [x[0] for x in lars_eval], label='LARS Training loss', color='C1', linestyle='solid')
    ax2.plot(range_eval_x, [x[1] for x in lars_eval], label='LARS Validation loss', color='C1', linestyle='dashed')
    ax2a.plot(range_eval_x, [x[2] for x in lars_eval], label='LARS Accuracy', color='C1', linestyle='dotted')
    ax2.plot(range_eval_x, [x[0] for x in sgd_eval], label='SGD Training loss', color='C2', linestyle='solid')
    ax2.plot(range_eval_x, [x[1] for x in sgd_eval], label='SGD Validation loss', color='C2', linestyle='dashed')
    ax2a.plot(range_eval_x, [x[2] for x in sgd_eval], label='SGD Accuracy', color='C2', linestyle='dotted')
    ax2.set_yscale('log')
    ax2.legend()
    ax2a.legend(loc=0)
    ax2.set_title(f'pred lr : {lr_predictor} epoch : {hyper_epoch_predictor} batch_size : {hyper_batch_size_predictor}')

    plt.tight_layout()
    plt.show()

    print(f'top-1 SAM : {sam_correct_1} LARS : {lars_correct_1} SGD : {sgd_correct_1}')
    print(f'top-5 SAM : {sam_correct_5} LARS : {lars_correct_5} SGD : {sgd_correct_5}')
